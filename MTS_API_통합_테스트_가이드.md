...
- ✅ **Phase 3**: 카카오 알림톡 (**완전 완료!** 🎉)
  - ✅ 템플릿 조회 기능 정상 작동
  - ✅ 변수 없는 템플릿 발송 성공 및 수신 확인 (TEST_INSPECT_001)
  - ✅ `inspection_status` 필드 추가 및 UI 업데이트 완료
  - ✅ 템플릿 상태 표시 개선 (승인됨 ✅, 등록됨, 검수중 ⏳, 반려됨 ❌)
  - ✅ **변수 포함 템플릿 발송 및 수신 확인 완료** (2025-11-11)
    - 템플릿 승인 완료 후 변수 치환 기능 재테스트 성공
    - 실제 메시지 수신 확인 (#{고객명}, #{날짜} 등)
  - ✅ **알림톡 → SMS 대체발송 테스트 완료** (2025-11-12)
    - tranType="SMS" 파라미터 정상 작동 확인
    - 알림톡 실패 시 자동 SMS 전환 확인
...
- ✅ **Phase 6**: 카카오 브랜드 메시지 (**핵심 타입 완료 - TEXT/IMAGE/WIDE + 버튼 조합 성공!** 🎉)
  ...
  - ✅ **변수분리방식 v1.1 전환 완료** (2025-11-10)
    - **기존 문제**: "전문방식" 사용 시 IMAGE/WIDE 타입에서 1030 에러 발생
    - **해결 방법**: message_variable, button_variable, image_variable 등을 별도 파라미터로 분리
    - **구현 위치**: `src/lib/mtsApi.ts` Line 1194-1298
  - ✅ **발송 테스트 결과** (2025-11-10 - 모두 성공!)
    - ✅ TEXT 타입: **성공** (result_code: 1000, 실제 수신 확인)
    - ✅ IMAGE 타입: **성공** (result_code: 1000, 실제 수신 확인)
    - ✅ WIDE 타입: **성공** (result_code: 1000, 실제 수신 확인)
    - ✅ TEXT + 버튼: **성공** (WL 타입 버튼, 최대 5개 지원)
    - ✅ IMAGE + 버튼: **성공** (이미지 + 버튼 조합)
    - ✅ WIDE + 버튼: **성공** (와이드 이미지 + 버튼 조합)
  - ✅ **신규 5개 타입 완전 완료! (2025-11-12)** 🎉
    - ✅ **WIDE_ITEM_LIST: 템플릿 등록 및 실제 발송 성공!** (2025-11-11)
      - UI 수정 완료 (배너 개념 제거, 첫 번째 아이템 안내 추가)
      - 이미지 업로드 API 구현 (`/upload-wide-item-image`, 2:1/1:1 비율 지원)
      - 템플릿 생성 성공 (MTS API 응답 code: "200")
      - ✅ **실제 메시지 발송 및 수신 확인 완료**
      - DB 저장 성공 (content 필드 NULL 오류 해결)
    - ✅ **CAROUSEL_FEED: 템플릿 등록 및 실제 발송 성공!** (2025-11-12)
      - 2:1 비율 이미지 자동 크롭 구현
      - 카드별 버튼 입력 UI (1-2개)
      - 2-6개 카드 제한 검증 추가
      - ✅ **실제 메시지 발송 및 수신 확인 완료**
    - ✅ **COMMERCE: 템플릿 등록 및 실제 발송 성공!** (2025-11-12)
      - 할인 타입 선택 UI (할인율 OR 정액할인)
      - MTS API commerce 객체 구조 적용
      - validMessageTypes 배열 수정 (send/route.ts Line 115)
      - ✅ **실제 메시지 발송 및 수신 확인 완료**
    - ✅ **CAROUSEL_COMMERCE: 템플릿 등록 및 실제 발송 성공!** (2025-11-12)
      - 2:1 비율 이미지 자동 크롭
      - 카드별 할인 타입 선택 UI
      - 2-6개 카드 제한 검증
      - validMessageTypes 배열 수정 (send/route.ts Line 115)
      - ✅ **실제 메시지 발송 및 수신 확인 완료**
    - ✅ **PREMIUM_VIDEO: 템플릿 등록 및 실제 발송 성공!** (2025-11-12)
      - Supabase 비디오 업로드 방식 제거
      - 카카오 TV URL 직접 입력 방식으로 변경
      - URL 형식 검증 추가 (프론트엔드 + 백엔드)
      - ✅ **실제 메시지 발송 및 수신 확인 완료**
    - ✅ **MTS API 구조 수정 완료** (2025-11-12)
      - CAROUSEL_FEED: `{ list: [...] }` 중첩 구조
      - COMMERCE: `{ commerce: {...} }` 객체 그룹화
      - CAROUSEL_COMMERCE: CAROUSEL + commerce 중첩
      - PREMIUM_VIDEO: `{ video: {...} }` 객체 그룹화
    - ✅ **타입 검증 배열 수정** (2025-11-12)
      - `src/app/api/messages/kakao/brand/send/route.ts` Line 115
...
>   - ✅ 알림톡 변수 치환 기능 완료
>   - ✅ **알림톡 → SMS 대체발송 테스트 성공!**
>     - tranType="SMS" 파라미터 정상 작동
>     - 알림톡 실패 시 자동 SMS 전환 확인
>     - metadata.tran_type, tran_message 저장 검증
>     - 실제 SMS 수신 확인
> - ✅ **Phase 6.0-6.4 완료** (2025-11-10): TEXT/IMAGE/WIDE + 버튼 조합 실제 발송 성공
>   - **핵심 성과**: 변수분리방식 v1.1 전환으로 1030 에러 완전 해결
>   - **테스트 완료**: TEXT, IMAGE, WIDE 타입 + 버튼(최대 5개) 모든 조합 성공
>   - **실제 수신 확인**: 모든 테스트 메시지 실제 수신 완료
> - ✅ **Phase 6.5-6.9 완전 완료!** (2025-11-12):
>   - ✅ **WIDE_ITEM_LIST**: 템플릿 등록 및 실제 발송 성공 (2025-11-11)
>   - ✅ **CAROUSEL_FEED**: 템플릿 등록 및 실제 발송 성공 (2025-11-12)
>   - ✅ **COMMERCE**: 템플릿 등록 및 실제 발송 성공 (2025-11-12)
>   - ✅ **CAROUSEL_COMMERCE**: 템플릿 등록 및 실제 발송 성공 (2025-11-12)
>   - ✅ **PREMIUM_VIDEO**: 템플릿 등록 및 실제 발송 성공 (2025-11-12)
>   - **핵심 해결**: COMMERCE/CAROUSEL_COMMERCE 타입 검증 배열 수정
...
| | 수신 대상 선택 (M/N/I) | ✅ | ✅ | ✅ | **완료** |
| | 자동 SMS 백업 타입 결정 | ✅ | ✅ | ✅ | **완료** |
| | 템플릿 관리 (등록/조회) | ✅ | ✅ | ⏸️ | **기본 기능 완료** |
| | WIDE_ITEM_LIST 타입 | ✅ | ✅ | ✅ | **완료** (2025-11-11) |
| | CAROUSEL_FEED 타입 | ✅ | ✅ | ❌ | **구현 완료, 테스트 대기** |
| | COMMERCE/VIDEO 타입 | ✅ | ✅ | ❌ | **구현 완료, 테스트 대기** |
...
**3.3 알림톡 발송 (변수 포함 템플릿) - ✅ 완료 (2025-11-11)**
- [x] 템플릿 선택 (TEST_VAR_002: #{고객명}, #{날짜}) ✅
- [x] 발송 API 성공 (code: "0000") ✅
- [x] ✅ **실제 메시지 수신 확인 완료** (템플릿 승인 후 테스트)
- [x] ✅ **변수 치환 기능 재테스트 완료**
...
- [x] ✅ 변수 포함 템플릿 발송 (템플릿 승인 후) - **완료 (2025-11-11)**
- [ ] 알림톡 → SMS 전환 발송 테스트
...
- [x] 와이드형 (FW) 발송 테스트 (구현 완료 - 2025-11-12)
...
## Phase 4.5: 카카오 친구톡 FW/FL/FC 타입 (2025-11-12 신규 추가)
...
- 이미지 링크 (imageLink) 지원
- 백엔드 API 업데이트: `sendMtsFriendtalk` 함수 FW 파라미터 추가
- API 라우트 업데이트: `/api/messages/kakao/friendtalk/send`

**✅ 와이드 아이템 리스트형 (FL)**
- UI 레이아웃: 헤더(20자) + 아이템 3-4개(제목 25자, 줄바꿈 1개)
- FL 타입 검증: 헤더 필수, 아이템 3-4개 필수
- 백엔드 API: `attachment.header`, `attachment.item_list` 구조
- ⏸️ TODO: 아이템 이미지 업로드 기능 구현 필요
...
- 더보기 링크 (moreLink) 지원
- FC 타입 검증: 캐러셀 2-6개 필수
- 백엔드 API: `attachment.carousel_list`, `attachment.more_link` 구조
- ⏸️ TODO: 캐러셀 이미지 업로드 기능 구현 필요
...
**1. FriendtalkTab.tsx** (`src/components/messages/FriendtalkTab.tsx`)
- Line 36: `type MessageType = 'FT' | 'FI' | 'FW' | 'FL' | 'FC'` 추가
- Line 49: `messageType` state 추가
- Line 68-82: FW/FL/FC 전용 state 추가 (headerText, listItems, carousels, moreLink)
- Line 287-293: 8AM-20PM 시간 제한 검증
- Line 449-509: 메시지 타입 선택 드롭다운 UI
- Line 547-639: FW 와이드형 레이아웃
- Line 642-764: FL 와이드 아이템 리스트형 레이아웃
- Line 633-817: FC 캐러셀형 레이아웃
- Line 820-678: 타입별 버튼 제한 로직

**2. kakaoApi.ts** (`src/utils/kakaoApi.ts`)
- Line 54-100: `FriendtalkSendRequest` 인터페이스 확장
  - `headerText?: string` (FL용)
  - `listItems?: Array<{ title, image }>` (FL용)
  - `carousels?: Array<{ content, image, buttons }>` (FC용)
  - `moreLink?: string` (FC용)

**3. mtsApi.ts** (`src/lib/mtsApi.ts`)
- Line 529-569: `sendMtsFriendtalk` 함수 시그니처 업데이트
  - FW/FL/FC 파라미터 추가 (headerText, listItems, carousels, moreLink)
- Line 619-654: FL 타입 처리 로직
  - `attachment.header`, `attachment.item_list` 구조 생성
- Line 635-654: FC 타입 처리 로직
  - `attachment.carousel_list`, `attachment.more_link` 구조 생성

**4. route.ts** (`src/app/api/messages/kakao/friendtalk/send/route.ts`)
- Line 37-54: API 문서 주석 업데이트 (FW/FL/FC 파라미터 추가)
- Line 64-67: Request body에서 FW/FL/FC 필드 추출
- Line 108-132: FL/FC 타입 검증 로직 추가
- Line 136-139: sendMtsFriendtalk 호출 시 FW/FL/FC 파라미터 전달
- Line 175-179: DB metadata에 FW/FL/FC 데이터 저장
...
- [x] 8AM-20PM 발송 시간 제한 (모든 타입) ✅
- [x] 실시간 문자 수 검증 ✅
- [x] 실시간 줄바꿈 개수 검증 ✅
- [x] DB 저장 확인 (metadata에 FW/FL/FC 데이터 포함) ✅
- [ ] 잔액 차감 확인 (친구톡 기본 단가 20원) ⏸️
...
// FW (와이드형)
{
  messageType: 'FW',
  attachment: {
    image: { img_url: "...", img_link: "..." },  // 필수
    button: [{ name, type, url_mobile, url_pc }]  // 최대 2개
  }
}
...
// FL (와이드 아이템 리스트형)
{
  messageType: 'FL',
  attachment: {
    header: "헤더 텍스트",  // 필수, 최대 20자
    item_list: [  // 3-4개
      { title: "아이템 제목", img_url: "..." }
    ],
    button: [...]  // 최대 2개
  }
}
...
- ✅ **검증 완료 타입** (2025-11-10):
  ...
  - WIDE + 버튼: 발송 성공
  - 🎉 **변수분리방식 v1.1 전환 완료!**
- 🆕 **신규 타입 구현 완료** (2025-11-11):
  - ✅ **WIDE_ITEM_LIST: 실제 발송 테스트 완료!** (2025-11-11)
    - 템플릿 생성 성공 (content 필드 NULL 오류 해결)
    - 이미지 업로드 API 구현 (2:1 첫 번째 아이템, 1:1 서브 아이템)
    - 실제 메시지 발송 및 수신 확인
    - 중요 발견: 첫 번째 리스트 아이템이 메시지 상단에 크게 표시됨
  - ✅ PREMIUM_VIDEO: UI + Supabase Storage 업로드 구현 완료, 테스트 대기
  - ✅ COMMERCE: UI + 상품 정보 필드 구현 완료, 테스트 대기
  - ✅ CAROUSEL_COMMERCE: UI 구현 완료, 테스트 대기
  - ✅ CAROUSEL_FEED: UI 구현 완료, 테스트 대기
- 📊 **DB 마이그레이션 완료**:
  - kakao_brand_templates 테이블에 7개 필드 추가
  - Supabase Storage kakao-videos 버킷 생성
  - 비디오 업로드 API 구현 완료

### 🔧 구현 현황 (v3.5 - 2025-11-11)
...
**6.10 브랜드 메시지 발송 (WIDE_ITEM_LIST 타입) - ✅ 테스트 완료 (2025-11-11)**

**테스트 일시**: 2025-11-11
**테스트 대상**: WIDE_ITEM_LIST 브랜드 메시지 타입
**테스트 결과**: ✅ **성공**

**테스트 단계:**
- [x] UI 수정 완료 (배너 개념 제거, 첫 번째 아이템 안내 추가) ✅
- [x] 이미지 업로드 API 구현 (`/api/messages/kakao/upload-wide-item-image`) ✅
  - 2:1 비율 이미지 (첫 번째 아이템용)
  - 1:1 비율 이미지 (서브 아이템용)
  - 카카오 이미지 서버 업로드
- [x] 템플릿 생성 (MTS API 응답 code: "200") ✅
- [x] DB 저장 (content 필드 NULL 오류 해결) ✅
- [x] 실제 메시지 발송 성공 ✅
- [x] 실제 메시지 수신 확인 ✅
  - [x] 첫 번째 리스트 아이템이 메시지 상단에 크게 표시됨
  - [x] 이미지 클릭 시 imageLink URL로 이동
  - [x] 메시지 내용 표시
  - [x] 버튼 클릭 동작 확인

**중요 발견 사항:**
- 🔍 "배너"로 착각했던 부분이 실제로는 첫 번째 리스트 아이템(mainWideItem)
- MTS API는 정상적으로 작동하며, WIDE_ITEM_LIST 타입의 첫 번째 아이템을 상단에 크게 표시함
- UI 안내 문구를 명확히 하여 향후 혼란 방지

**해결된 문제:**
- ✅ content 필드 NULL 오류: 템플릿 생성 시 content 필드에 실제 내용 저장하도록 수정
- ✅ 이미지 업로드: 별도 API 엔드포인트 구현으로 2:1/1:1 비율 이미지 업로드 지원
- ✅ UI 혼란: "배너" 개념 제거, "첫 번째 아이템" 명확화
...
### 테스트 시나리오 6.5: WIDE_ITEM_LIST 타입 (2025-11-11 신규)

**목적**: 와이드 아이템 리스트형 브랜드 메시지 템플릿 등록 및 발송 테스트

**사전 조건:**
- [ ] 테스트 이미지 준비 (JPG/PNG, 500KB 이하, 800x400px 권장)
- [ ] 브랜드 발신프로필 선택 완료

#### 📝 테스트 케이스 6.5.1: 템플릿 등록 (이미지 업로드)

**테스트 단계:**
1. "카카오/네이버 톡톡" 탭 → "브랜드 메시지 템플릿 관리" → "템플릿 등록"
2. 메시지 타입: **WIDE_ITEM_LIST** 선택
3. 템플릿 이름 입력: "와이드리스트_테스트_001"
4. 메시지 내용 입력 (76자 제한):
   ```
   특별 할인 상품을 확인하세요!
   ```
5. **이미지 업로드**:
   ...
   - 테스트 이미지 선택 (wide_item_list_sample.jpg)
   - 업로드 성공 확인 (미리보기 표시)
6. **이미지 링크 URL** 입력: `https://example.com/promotion`
7. (선택) 버튼 추가:
   ```javascript
   {
     ordering: 1,
     ...
     linkPc: 'https://example.com/detail'
   }
   ```
8. "등록하기" 버튼 클릭

**검증 포인트:**
- [ ] 카카오 이미지 서버 업로드 성공 (DevTools Network 탭 확인)
- [ ] API 응답: `{ success: true, template_code: 'BR...' }`
- [ ] Supabase `kakao_brand_templates` 테이블 확인:
  - `chat_bubble_type = 'WIDE_ITEM_LIST'`
  - `image_url` 필드에 카카오 이미지 URL 저장
  - `image_link` 필드에 입력한 URL 저장
  - `buttons` JSONB 필드에 버튼 배열 저장
- [ ] 성공 알림: "템플릿이 등록되었습니다. 검수 승인 후 사용 가능합니다."

#### 📝 테스트 케이스 6.5.2: 템플릿 발송 (검수 승인 후)

**사전 조건:**
- [ ] MTS 검수팀 승인 완료 (status='A')
- [ ] 테스트 수신번호 채널 친구 추가 완료

**테스트 단계:**
1. "브랜드톡" 탭 이동
2. 발신프로필 선택
3. 템플릿 선택: "와이드리스트_테스트_001"
4. 수신 대상: `I` (전체+채널친구) - 기본값
5. 수신번호 입력: `01012345678`
6. (선택) 변수 치환 (있는 경우)
7. SMS 백업: 체크 해제
8. "전송" 버튼 클릭
...
```javascript
{
  ...
  targeting: "I",
  message_variable: {
    message: "특별 할인 상품을 확인하세요!"
  },
  image_variable: [
    {
      img_url: "https://mud-kage.kakao.com/...",
      img_link: "https://example.com/promotion"
    }
  ],
  button_variable: {
    link1: "https://example.com/detail"
  }
}
```
...
- [ ] `message_logs` 테이블 레코드 생성:
  - `message_type = 'BRAND'`
  - `brand_template_code = 'BR...'`
  - `status = 'pending'`
- [ ] `transactions` 테이블: -20원 차감
- [ ] **실제 메시지 수신 확인**:
  - [ ] 와이드 이미지 표시 확인
  - [ ] 이미지 클릭 → `image_link` URL 이동 확인
  - [ ] 메시지 내용 표시 확인
  - [ ] 버튼 클릭 동작 확인

---

### 테스트 시나리오 6.6: PREMIUM_VIDEO 타입 (2025-11-11 신규)

**목적**: 프리미엄 비디오형 브랜드 메시지 템플릿 등록 및 발송 테스트

**사전 조건:**
- [ ] 테스트 비디오 준비 (MP4/MOV/AVI/WEBM, 200MB 이하)
- [ ] 썸네일 이미지 준비 (JPG/PNG, 500KB 이하)

#### 📝 테스트 케이스 6.6.1: 템플릿 등록 (비디오 + 썸네일 업로드)

**테스트 단계:**
1. "카카오/네이버 톡톡" 탭 → "브랜드 메시지 템플릿 관리" → "템플릿 등록"
2. 메시지 타입: **PREMIUM_VIDEO** 선택
3. 템플릿 이름 입력: "비디오_테스트_001"
4. 메시지 내용 입력:
   ```
   신제품 출시 영상을 확인하세요!
   ```
5. **동영상 업로드**:
   - "동영상 선택" 버튼 클릭
   - 테스트 비디오 선택 (sample_video.mp4)
   - 업로드 진행률 확인
   - 업로드 완료 확인 (파일명 표시)
6. **썸네일 이미지 업로드**:
   - "썸네일 이미지 선택" 버튼 클릭
   - 테스트 이미지 선택 (thumbnail.jpg)
   - 업로드 성공 확인 (미리보기 표시)
7. (선택) 버튼 추가
8. "등록하기" 버튼 클릭

**검증 포인트:**
- [ ] 비디오 업로드 API 호출: `/api/messages/kakao/upload-video`
  - Request: FormData with `file`
  - Response: `{ success: true, url: 'https://...supabase.co/storage/...' }`
- [ ] 썸네일 업로드 API 호출: `/api/messages/kakao/upload-image`
- [ ] Supabase Storage `kakao-videos` 버킷 확인:
  - 파일 경로: `{userId}/{timestamp}_filename.mp4`
  - 공개 URL 생성 확인
- [ ] Supabase `kakao_brand_templates` 테이블 확인:
  - `chat_bubble_type = 'PREMIUM_VIDEO'`
  - `video_url` 필드에 Supabase 비디오 URL 저장
  - `thumbnail_url` 필드에 썸네일 이미지 URL 저장
- [ ] 파일 크기 제한 검증:
  - 200MB 초과 파일 업로드 시 에러 메시지 확인
- [ ] 파일 형식 검증:
  - JPG 파일 비디오로 업로드 시 에러 메시지 확인

#### 📝 테스트 케이스 6.6.2: 템플릿 발송

**테스트 단계:**
1. "브랜드톡" 탭 이동
2. 템플릿 선택: "비디오_테스트_001"
3. 수신번호 입력
4. "전송" 버튼 클릭
...
```javascript
{
  ...
  targeting: "I",
  message_variable: {
    message: "신제품 출시 영상을 확인하세요!"
  },
  video_variable: {
    video_url: "https://...supabase.co/storage/...",
    thumbnail_url: "https://mud-kage.kakao.com/..."
  }
}
```
...
### 테스트 시나리오 6.7: COMMERCE 타입 (2025-11-11 신규)

**목적**: 커머스형 브랜드 메시지 템플릿 등록 및 발송 테스트

**사전 조건:**
- [ ] 상품 이미지 준비 (JPG/PNG, 500KB 이하)

#### 📝 테스트 케이스 6.7.1: 템플릿 등록 (상품 정보 입력)

**테스트 단계:**
1. "카카오/네이버 톡톡" 탭 → "브랜드 메시지 템플릿 관리" → "템플릿 등록"
2. 메시지 타입: **COMMERCE** 선택
3. 템플릿 이름 입력: "커머스_테스트_001"
4. 메시지 내용 입력:
   ```
   한정 특가 상품을 놓치지 마세요!
   ```
...
**검증 포인트:**
- [ ] 상품 이미지 업로드 API 호출 성공
- [ ] Supabase `kakao_brand_templates` 테이블 확인:
  - `chat_bubble_type = 'COMMERCE'`
  - `image_url` 필드에 상품 이미지 URL 저장
  - `commerce_title = '프리미엄 노트북'`
  - `regular_price = 1500000`
  - `discount_price = 1200000`
  - `discount_rate = 20`
  - `discount_fixed = 300000`
- [ ] 필수 필드 누락 시 에러:
  - 상품명 미입력 → "상품명을 입력해주세요" 에러
  - 정가 미입력 → "정가를 입력해주세요" 에러
  - 이미지 미업로드 → "상품 이미지를 업로드해주세요" 에러

#### 📝 테스트 케이스 6.7.2: 템플릿 발송

**테스트 단계:**
1. "브랜드톡" 탭 이동
2. 템플릿 선택: "커머스_테스트_001"
3. 수신번호 입력
4. "전송" 버튼 클릭
...
- [ ] **실제 메시지 수신 확인**:
  ...
  - [ ] 할인가 표시 확인 (₩1,200,000)
  - [ ] 할인율 표시 확인 (20%)
  - [ ] 할인 금액 표시 확인 (₩300,000 할인)

---

### 테스트 시나리오 6.8: CAROUSEL_COMMERCE 타입 (2025-11-11 신규)

**목적**: 캐러셀 커머스형 브랜드 메시지 템플릿 등록 및 발송 테스트

**참고**: CAROUSEL_COMMERCE는 COMMERCE와 동일한 UI 및 필드를 사용하며, 캐러셀 구조로 표시됩니다.

#### 📝 테스트 케이스 6.8.1: 템플릿 등록

**테스트 단계:**
1. 메시지 타입: **CAROUSEL_COMMERCE** 선택
2. 템플릿 이름: "캐러셀커머스_테스트_001"
3. 메시지 내용, 상품 이미지, 상품 정보 입력 (6.7.1과 동일)
4. "등록하기" 버튼 클릭

**검증 포인트:**
- [ ] Supabase 테이블 확인: `chat_bubble_type = 'CAROUSEL_COMMERCE'`
- [ ] 기타 필드는 COMMERCE와 동일하게 저장

#### 📝 테스트 케이스 6.8.2: 템플릿 발송

**검증 포인트:**
- [ ] MTS API 호출 성공
- [ ] **실제 메시지 수신 확인**:
  - [ ] 캐러셀 UI로 표시 확인
  - [ ] 스와이프 동작 확인 (여러 상품 있을 경우)
  - [ ] 상품 정보 표시 확인

---

### 테스트 시나리오 6.9: CAROUSEL_FEED 타입 (2025-11-11 신규)

**목적**: 캐러셀 피드형 브랜드 메시지 템플릿 등록 및 발송 테스트

**참고**: CAROUSEL_FEED는 여러 개의 피드 아이템을 캐러셀 형태로 표시합니다.

#### 📝 테스트 케이스 6.9.1: 템플릿 등록

**테스트 단계:**
1. 메시지 타입: **CAROUSEL_FEED** 선택
2. 템플릿 이름: "캐러셀피드_테스트_001"
3. 메시지 내용 입력
4. (타입별 필수 필드 입력 - MTS API 문서 참조)
5. "등록하기" 버튼 클릭

**검증 포인트:**
- [ ] Supabase 테이블 확인: `chat_bubble_type = 'CAROUSEL_FEED'`
- [ ] MTS 검수 승인 대기

#### 📝 테스트 케이스 6.9.2: 템플릿 발송
...
### 테스트 시나리오 6.12: 에러 처리 테스트

**목적**: 다양한 에러 상황에서 적절한 에러 메시지 표시 확인

**테스트 케이스 6.12.1: 에러 1028 (타게팅 권한 없음)**
...
**테스트 케이스 6.12.4: 시간 제한 (08:00~20:00 외)**
...
### WIDE_ITEM_LIST 타입 ✅ (2025-11-11 완료)
- [x] 템플릿 등록 (이미지 업로드 + imageLink) ✅
- [x] 카카오 이미지 서버 업로드 성공 확인 ✅
  - [x] 2:1 비율 첫 번째 아이템 이미지 ✅
  - [x] 1:1 비율 서브 아이템 이미지 ✅
- [x] MTS 검수 승인 ✅
- [x] 메시지 발송 (targeting='I') ✅
- [x] 실제 수신 확인: ✅
  - [x] 첫 번째 리스트 아이템이 상단에 크게 표시됨 ✅
  - [x] 이미지 클릭 → imageLink URL 이동 ✅
  - [x] 메시지 내용 표시 ✅
- [x] 버튼 추가 시 동작 확인 ✅
- [x] **content 필드 NULL 오류 해결** ✅
- [x] **이미지 업로드 API 구현** (`/upload-wide-item-image`) ✅
- [x] **UI 개선** (배너 개념 제거, 안내 문구 추가) ✅

### PREMIUM_VIDEO 타입 (2025-11-11 신규)
- [ ] 템플릿 등록 (동영상 + 썸네일 업로드)
- [ ] Supabase Storage 업로드 성공 확인
  - [ ] 비디오: kakao-videos 버킷
  - [ ] 썸네일: 카카오 이미지 서버
- [ ] 파일 크기/형식 제한 검증
  - [ ] 200MB 초과 → 에러 메시지
...
### CAROUSEL_COMMERCE 타입 (2025-11-11 신규)
- [ ] 템플릿 등록 (COMMERCE와 동일 UI)
- [ ] MTS 검수 승인
- [ ] 메시지 발송
- [ ] 실제 수신 확인:
  - [ ] 캐러셀 UI 표시
...
### CAROUSEL_FEED 타입 (2025-11-11 신규)
- [ ] 템플릿 등록
- [ ] MTS 검수 승인
- [ ] 메시지 발송
- [ ] 실제 수신 확인:
