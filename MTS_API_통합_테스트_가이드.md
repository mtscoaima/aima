...
## 🚨 현재 테스트 현황 (2025-11-04 체크리스트 초기화)
...
### 📊 테스트 진행 상태 (v2.0 - 체크리스트 초기화)
- ⏸️ **Phase 1-2**: SMS/LMS/MMS (재테스트 대기)
- ⏸️ **Phase 3**: 카카오 알림톡 (재테스트 대기)
- ⏸️ **Phase 4**: 카카오 친구톡 (재테스트 대기)
- ⏸️ **Phase 5**: 네이버 톡톡 (재테스트 대기)
- ⏸️ **Phase 6**: 카카오 브랜드 메시지 (재테스트 대기, UI 재설계 완료)

> ✅ **문서 업데이트 (2025-11-04)**: 전체 코드베이스 실사 완료, 모든 체크리스트 초기화
> - 실제 구현 상태 확인 완료 (mtsApi.ts: 1850줄, 19개 함수)
> - API 엔드포인트 전수 조사 완료 (12개 메시지 관련 API)
> - 모든 테스트 항목 초기화하여 재테스트 준비
...
**3.2 알림톡 발송 (변수 치환 포함)**
- [ ] 템플릿 선택 (변수 포함: #{고객명}, #{날짜} 등)
- [ ] 변수 개수 표시 확인 (예: "2개의 변수가 존재합니다")
- [ ] 수신자 정보 입력 시 변수 자동 치환
- [ ] **템플릿 내용 readOnly 확인** (수정 불가)
- [ ] 발송 버튼 클릭
- [ ] MTS API 응답 코드 `0000` 또는 `1000` 확인
- [ ] DB 저장 확인 (message_logs)
- [ ] 잔액 13원 차감 확인 (transactions)
- [ ] **중요**: 템플릿 본문 외 텍스트 추가 금지 (3016 에러 방지)
...
**올바른 예:**
- 템플릿: `[테스트] 검수 요청 로깅 테스트입니다.\n감사합니다.`
- 발송: `[테스트] 검수 요청 로깅 테스트입니다.\n감사합니다.` ✅

**잘못된 예:**
- 템플릿: `[테스트] 검수 요청 로깅 테스트입니다.\n감사합니다.`
- 발송: `[테스트] 안녕하세요 #{고객명}님, 검수 요청 로깅 테스트입니다.\n감사합니다.` ❌
- 에러: 3016 (템플릿 본문 불일치)

**에러 코드 로깅 확인:**
- [ ] DB metadata에 error_code 저장 확인
- [ ] 주요 에러: 3016(템플릿불일치), 3019(톡유저아님), ER17(미등록발신번호)
...
## 최종 체크리스트 (v2.2 - 메시지 전송 로직 개선)

### Phase 1-2: SMS/LMS/MMS
- [ ] SMS 발송 (90바이트 이하, 25원) + 변수 치환 (#{이름})
- [ ] LMS 발송 (90바이트 초과, 50원) + 변수 치환 (#{고객명}, #{상품명})
- [ ] API 엔드포인트 분리 확인:
  - [ ] SMS → `/sndng/sms/sendMessage` (Console.log 확인)
  - [ ] LMS/MMS → `/sndng/mms/sendMessage` (Console.log 확인)
- [ ] MMS 이미지 업로드 (PNG→JPEG, 640×480px, 300KB)
- [ ] MMS 발송 (이미지 포함, 100원)
...
- [ ] 변수 치환 발송 (#{고객명}, #{날짜})
- [ ] 변수 개수 표시 확인
- [ ] 템플릿 내용 readOnly 확인 (3016 에러 방지)
- [ ] 버튼 기능
- [ ] SMS 전환 발송
- [ ] 잔액 13원 차감 확인
- [ ] MTS API 응답 성공 (code: "0000" 또는 "1000")
- [ ] metadata에 error_code 저장 확인
- [ ] 실제 메시지 수신 확인
