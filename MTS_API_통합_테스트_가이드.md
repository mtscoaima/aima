...
**4.2 이미지형 친구톡 (자동 감지 - FI)** ✅ 완료
- [x] Textarea 하단 이미지 아이콘 버튼 클릭 → 이미지 업로드 섹션 토글 ✅
- [x] "이미지 선택" 버튼으로 JPG/PNG 파일 선택 ✅
- [x] Kakao 이미지 업로드 API 호출 및 Kakao 서버 URL 저장 ✅
- [x] 이미지 미리보기 확인 (썸네일, 파일명, 크기) ✅
- [x] 이미지 규격 안내 문구 확인 (2:1 비율, 500KB 제한) ✅
- [x] 메시지 내용 입력 (치환문구 포함: #{이름}, #{날짜}) ✅
- [x] 발송 버튼 클릭 ✅
- [x] Console.log에서 "[친구톡] 메시지 타입 자동 감지: FI (이미지형)" 확인 ✅
- [x] MTS API 응답 성공 (code: "0000") ✅
- [x] 이미지 attachment 구조 수정 (array → object) ✅
- [x] ✅ 실제 메시지 수신 확인 - **완료!**
  - **이슈**: MTS 서버 이미지를 Kakao 서버에서 접근 불가
  - **해결**: Kakao 전용 이미지 업로드 API 구현 완료
  - **결과**: 이미지 + 문구 정상 전송 및 수신 확인
...
### 🔍 발견된 이슈 - Phase 4.2 (이미지형 친구톡)

#### 문제 상황
- **증상**: 이미지형 친구톡 (FI) 메시지가 실제로 수신되지 않음
- **API 응답**: 정상 (code: "0000", received_at 존재)
- **텍스트형 친구톡 (FT)**: 정상 작동 ✅
- **변수 치환**: 정상 작동 ✅
...
1. **MMS (문자메시지)** ✅ 정상 작동
   - 업로드 API: `/img/upload_image`
   - 저장 위치: **MTS 서버**
   - 이미지 경로: 상대 경로 (`/2025/11/05/...`)
   - MTS가 내부적으로 처리하므로 정상 작동

2. **Friendtalk (친구톡)** ❌ 이미지 미수신
   - 현재 사용 중: `/img/upload_image` (MMS용)
   - 저장 위치: **MTS 서버**
   - 문제: Kakao 서버에서 MTS 서버 이미지에 접근 불가
   - 필요: **Kakao 서버 이미지** (`https://mud-kage.kakao.com/...`)

**테스트 결과 분석**

| 테스트 항목 | 상대 경로 | 전체 URL | 결과 |
|------------|----------|---------|------|
| 이미지 URL 형식 | `/2025/11/05/...` | `https://api.mtsco.co.kr/2025/11/05/...` | 둘 다 실패 |
| URL 접근 테스트 | - | `curl` 결과: 404 Not Found | 접근 불가 |
| API 응답 | code: "0000" | code: "0000" | 성공 |
| 실제 수신 | ❌ | ❌ | 미수신 |
...
1. **새 API 엔드포인트**: `/api/messages/kakao/upload-image`
2. **MTS API 사용**: `https://talks.mtsco.co.kr/mts/api/image/alimtalk/template`
3. **참고 문서**: `docs/연동규격서md/비즈API/카카오 이미지 업로드 API 20210629.md`
4. **파라미터**:
   - `senderKey`: 발신 프로필 키
   - `image`: 업로드할 이미지 파일 (binary)
5. **응답**: Kakao 서버 URL (`https://mud-kage.kakao.com/...`)
6. **이미지 규격**:
   - 가로 500px 이상
   - 가로:세로 비율 2:1
   - 파일 형식: JPG, PNG
...
1. `src/lib/mtsApi.ts` (Line 617-624)
   - 이미지 attachment 구조 수정: array → single object
   - MTS API 규격에 맞게 수정 완료 ✅

**구현 완료된 코드** ✅

1. `src/app/api/messages/kakao/upload-image/route.ts` (신규 생성) ✅
   - Kakao 전용 이미지 업로드 엔드포인트
   - MTS API: `https://talks.mtsco.co.kr/mts/api/image/alimtalk/template`
   - 파라미터: `senderKey`, `image` (multipart/form-data)
   - Sharp 이미지 최적화: 2:1 비율, 최소 500px, 최대 500KB
   - 반환: Kakao 서버 URL (`https://mud-kage.kakao.com/...`)

2. `src/components/messages/FriendtalkTab.tsx` (수정 완료) ✅
   - Line 183-186: senderKey 검증 추가
   - Line 191: senderKey를 FormData에 추가
   - Line 200: `/api/messages/kakao/upload-image` 호출로 변경
   - Line 225: Kakao 서버 URL 저장
   - Line 558-573: 이미지 규격 안내 문구 추가
   - 상세 로깅 추가 (senderKey, 파일명, 파일 크기)

#### 테스트 완료! ✅
- [x] 이미지 업로드 테스트 (Kakao 서버 URL 획득 확인) ✅
- [x] 이미지형 친구톡 (FI) 발송 테스트 ✅
- [x] 실제 메시지 수신 확인 ✅
- [x] 이미지 비율 안내 문구 표시 확인 ✅

#### 최종 결과
- ✅ Kakao 서버 이미지 URL 획득 (`https://mud-kage.kakao.com/...`)
- ✅ 이미지형 친구톡 (FI) 정상 발송
- ✅ 실제 메시지 수신 확인 (이미지 + 문구)
- ✅ 사용자 안내: 2:1 비율 권장, 이미지 자동 잘림 경고
...
- [x] 텍스트형 (FT) - 비광고 ✅
- [x] 이미지형 (FI) - 이미지 업로드 + 변수 치환 (#{이름}, #{날짜}) ✅
- [x] 치환문구 버튼 동작 확인 ✅
- [x] 이미지 규격 안내 문구 추가 (2:1 비율) ✅
- [ ] 이미지 클릭 링크 (imageLink) - 미테스트
- [ ] 와이드형 (FW) - 미테스트
- [ ] 광고형 (ad_flag=Y) - 미테스트
- [x] 잔액 20원 차감 확인 ✅
- [x] MTS API 응답 성공 (code: "0000") ✅
- [x] 실제 메시지 수신 확인 (FT, FI 모두) ✅
- [x] Kakao 전용 이미지 업로드 API 구현 완료 ✅
