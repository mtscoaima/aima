...
const DEFAULT_SECU_KEY = "4261358467855134";  // 기본 암호화 키 (16자리)
const DEFAULT_SEED_IV = "3AD8D3E22F132AC7";   // 기본 IV (16자리)
const HEADER_PREFIX = "KMC000002-7F36131B";   // 암호화 데이터 헤더
const EXTEND_VAR = "0000000000000000";        // 확장변수 (고정값)

// ============================================================================
// S-Box 테이블 (KISA SEED 표준)
// ============================================================================
...
function getB0(A: number): number { return 0xff & A; }
function getB1(A: number): number { return 0xff & (A >>> 8); }
function getB2(A: number): number { return 0xff & (A >>> 16); }
function getB3(A: number): number { return 0xff & (A >>> 24); }
...
function seedRound(
  ...
  T[1] = (SS0[getB0(T[1])] ^ SS1[getB1(T[1])] ^ SS2[getB2(T[1])] ^ SS3[getB3(T[1])]) >>> 0;
  T[0] = (T[0] + T[1]) >>> 0;
  T[0] = (SS0[getB0(T[0])] ^ SS1[getB1(T[0])] ^ SS2[getB2(T[0])] ^ SS3[getB3(T[0])]) >>> 0;
  T[1] = (T[1] + T[0]) >>> 0;
  T[1] = (SS0[getB0(T[1])] ^ SS1[getB1(T[1])] ^ SS2[getB2(T[1])] ^ SS3[getB3(T[1])]) >>> 0;
  T[0] = (T[0] + T[1]) >>> 0;
...
function seedRoundKey(roundKey: number[], userKey: Uint8Array): void {
  ...
  K[0] = ((userKey[0] << 24) | (userKey[1] << 16) | (userKey[2] << 8) | userKey[3]) >>> 0;
  K[1] = ((userKey[4] << 24) | (userKey[5] << 16) | (userKey[6] << 8) | userKey[7]) >>> 0;
  K[2] = ((userKey[8] << 24) | (userKey[9] << 16) | (userKey[10] << 8) | userKey[11]) >>> 0;
  K[3] = ((userKey[12] << 24) | (userKey[13] << 16) | (userKey[14] << 8) | userKey[15]) >>> 0;
  ...
  for (let i = 0; i < 16; i++) {
    ...
    roundKey[idx] = (SS0[getB0(T[0])] ^ SS1[getB1(T[0])] ^ SS2[getB2(T[0])] ^ SS3[getB3(T[0])]) >>> 0;
    roundKey[idx + 1] = (SS0[getB0(T[1])] ^ SS1[getB1(T[1])] ^ SS2[getB2(T[1])] ^ SS3[getB3(T[1])]) >>> 0;
...
// SEED 블록 암호화
function seedEncryptBlock(roundKey: number[], plainText: Uint8Array, cipherText: Uint8Array): void {
  const LR = new Array(4);
  const T = new Array(2);

  LR[0] = ((plainText[0] << 24) | (plainText[1] << 16) | (plainText[2] << 8) | plainText[3]) >>> 0;
  LR[1] = ((plainText[4] << 24) | (plainText[5] << 16) | (plainText[6] << 8) | plainText[7]) >>> 0;
  LR[2] = ((plainText[8] << 24) | (plainText[9] << 16) | (plainText[10] << 8) | plainText[11]) >>> 0;
  LR[3] = ((plainText[12] << 24) | (plainText[13] << 16) | (plainText[14] << 8) | plainText[15]) >>> 0;
  ...
  for (let i = 0; i < 16; i++) {
    seedRound(T, LR, (i + 0) % 4, (i + 1) % 4, (i + 2) % 4, (i + 3) % 4, roundKey, i * 2);
  }
...
// SEED 블록 복호화
function seedDecryptBlock(roundKey: number[], cipherText: Uint8Array, plainText: Uint8Array): void {
  const LR = new Array(4);
  const T = new Array(2);

  LR[0] = ((cipherText[0] << 24) | (cipherText[1] << 16) | (cipherText[2] << 8) | cipherText[3]) >>> 0;
  LR[1] = ((cipherText[4] << 24) | (cipherText[5] << 16) | (cipherText[6] << 8) | cipherText[7]) >>> 0;
  LR[2] = ((cipherText[8] << 24) | (cipherText[9] << 16) | (cipherText[10] << 8) | cipherText[11]) >>> 0;
  LR[3] = ((cipherText[12] << 24) | (cipherText[13] << 16) | (cipherText[14] << 8) | cipherText[15]) >>> 0;
  ...
  for (let i = 15; i >= 0; i--) {
    seedRound(T, LR, (i + 2) % 4, (i + 3) % 4, (i + 0) % 4, (i + 1) % 4, roundKey, i * 2);
  }
...
function addPKCS5Padding(data: Uint8Array): Uint8Array {
  const blockSize = 16;
...
function seedCBCEncrypt(plainData: Uint8Array, key: Uint8Array, iv: Uint8Array): Uint8Array {
  if (key.length !== 16) throw new Error("SEED key must be 16 bytes");
  if (iv.length !== 16) throw new Error("SEED IV must be 16 bytes");

  const padded = addPKCS5Padding(plainData);
  const roundKey = new Array(32);
  seedRoundKey(roundKey, key);

  const encrypted = new Uint8Array(padded.length);
  const prevBlock = new Uint8Array(16);
  const tempBlock = new Uint8Array(16);

  iv.forEach((byte, i) => (prevBlock[i] = byte));
  ...
  for (let i = 0; i < padded.length; i += 16) {
    ...
    for (let j = 0; j < 16; j++) {
      tempBlock[j] = currentBlock[j] ^ prevBlock[j];
    }

    // Encrypt
    const encryptedBlock = new Uint8Array(16);
    seedEncryptBlock(roundKey, tempBlock, encryptedBlock);

    // Copy to result
    encrypted.set(encryptedBlock, i);

    // Save for next iteration
    encryptedBlock.forEach((byte, j) => (prevBlock[j] = byte));
  }

  return encrypted;
}

function seedCBCDecrypt(encryptedData: Uint8Array, key: Uint8Array, iv: Uint8Array): Uint8Array {
  if (key.length !== 16) throw new Error("SEED key must be 16 bytes");
  if (iv.length !== 16) throw new Error("SEED IV must be 16 bytes");
  if (encryptedData.length % 16 !== 0) throw new Error("Encrypted data length must be multiple of 16");
...
/**
 * 키/IV 처리 (JAR의 getSecuIV 로직)
 */
function processKeyOrIV(input: string): string {
  if (input.length > 16) {
    return input.substring(input.length - 16);
  } else {
 ...
 * KMC SEED CBC 암호화
 * @param plaintext 평문
 * @param key 암호화 키 (기본값: DEFAULT_SECU_KEY)
 * @param iv IV (기본값: DEFAULT_SEED_IV)
 * @returns Hex 문자열로 인코딩된 암호문
 */
export function kmcEncrypt(
  plaintext: string,
  key: string = DEFAULT_SECU_KEY,
  iv: string = DEFAULT_SEED_IV
): string {
  try {
    // EUC-KR로 인코딩
    ...
    // 키와 IV 처리
    const processedKey = processKeyOrIV(key);
    const processedIV = processKeyOrIV(iv);

    const keyBytes = new Uint8Array(Buffer.from(processedKey, "utf8"));
    const ivBytes = new Uint8Array(Buffer.from(processedIV, "utf8"));

    // SEED CBC 암호화
    const encrypted = seedCBCEncrypt(new Uint8Array(plainBytes), keyBytes, ivBytes);

    // Hex 문자열로 변환
    return bytesToHex(encrypted);
  } catch (error) {
    console.error("KMC encryption error:", error);
...
/**
 * KMC SEED CBC 복호화
 * @param ciphertext Hex 문자열로 인코딩된 암호문
 * @param key 암호화 키 (기본값: DEFAULT_SECU_KEY)
 * @param iv IV (기본값: DEFAULT_SEED_IV)
 * @returns 복호화된 평문
 */
export function kmcDecrypt(
  ciphertext: string,
  key: string = DEFAULT_SECU_KEY,
  iv: string = DEFAULT_SEED_IV
): string {
  try {
    ...
    // SEED CBC 복호화
    const decrypted = seedCBCDecrypt(encryptedBytes, keyBytes, ivBytes);

    // EUC-KR로 디코딩
    return iconv.decode(Buffer.from(decrypted), "euc-kr").trim();
  } catch (error) {
    console.error("KMC decryption error:", error);
    throw new Error("복호화 중 오류가 발생했습니다.");
  }
}

/**
 * SHA-256 HMAC 생성 (JAR의 getMsg 로직)
 * @param data 입력 데이터
 * @returns Hex 문자열로 인코딩된 해시값
 */
export function kmcHmac(data: string): string {
...
export interface KmcRequestParams {
  cpId: string;           // 고객사 ID
  urlCode: string;        // URL 코드
  certNum: string;        // 요청번호
  date: string;           // 요청일시 (YYYYMMDDhhmmss)
  certMet?: string;       // 본인확인방법 (M/P/C)
  plusInfo?: string;      // 추가 데이터
}

export interface KmcResponseData {
  certNum: string;        // 요청번호
  date: string;           // 요청일시
  ci: string;             // 연계정보
  ...
  gender: string;         // 성별 (0: 남자, 1: 여자)
  nation: string;         // 내/외국인 (0: 내국인, 1: 외국인)
  name: string;           // 성명
  result: string;         // 결과값 (Y/N/F)
  certMet: string;        // 본인확인방법
  ip: string;             // IP 주소
  di: string;             // 중복가입확인정보
  plusInfo?: string;      // 추가 데이터
}
 ...
 * 2단계 암호화 적용
 */
export function generateTrCert(params: KmcRequestParams): string {
  const {
    cpId,
    urlCode,
    certNum,
  ...
  // 2차 암호화
  const enc2 = kmcEncrypt(secondData);

  // 헤더 추가
  return HEADER_PREFIX + enc2;
}

/**
 * KMC 응답 데이터(rec_cert) 파싱
 * 2단계 복호화 적용
 */
export function parseRecCert(encryptedData: string): KmcResponseData {
  try {
    // 1차 복호화
    ...
    if (parts1.length < 3) {
      throw new Error("Invalid rec_cert format (1st parse)");
    }

    const encPara = parts1[0];   // 암호화된 파라미터
    const encMsg1 = parts1[1];   // HMAC 값
    ...
    if (parts2.length < 19) {
      throw new Error("Invalid rec_cert format (2nd parse)");
    }

    // CI, DI 추가 복호화
    const ci = parts2[2] ? kmcDecrypt(parts2[2]) : "";
    const di = parts2[17] ? kmcDecrypt(parts2[17]) : "";
    ...
    console.error("Parse rec_cert error:", error);
