...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: newUser, error: insertError } = await supabase
      ...
      .insert({
        email,
        password: "NAVER_USER", // 네이버 사용자는 비밀번호 없음 (구분용)
        name,
        ...
        updated_at: now,
        last_login_at: now,
        email_verified: true, // 네이버로 가입한 경우 이메일 인증됨
    ...
    const documentsInfo: { [key: string]: UploadedFile } = {};

    if (userType === "general") {
      // 사업자등록증 업로드
      if (businessRegistration) {
        try {
          const fileName = `business_registration_${
            newUser.id
          }_${Date.now()}.${businessRegistration.name.split(".").pop()}`;
          ...
          if (uploadError) {
            console.error("Business registration upload error:", uploadError);
          } else {
            const { data: publicUrl } = supabase.storage
              .from("documents")
              .getPublicUrl(filePath);

            documentsInfo.businessRegistration = {
              url: publicUrl.publicUrl,
              name: businessRegistration.name,
              size: businessRegistration.size,
              type: businessRegistration.type,
            };
          }
        } catch (uploadError) {
          console.error("Business registration upload exception:", uploadError);
        }
      }
      ...
      if (employmentCertificate) {
        try {
          const fileName = `employment_certificate_${
            newUser.id
          }_${Date.now()}.${employmentCertificate.name.split(".").pop()}`;
          const filePath = `documents/${newUser.id}/${fileName}`;
          ...
          if (uploadError) {
            console.error("Employment certificate upload error:", uploadError);
          } else {
            const { data: publicUrl } = supabase.storage
              .from("documents")
              .getPublicUrl(filePath);

            documentsInfo.employmentCertificate = {
              url: publicUrl.publicUrl,
              name: employmentCertificate.name,
              size: employmentCertificate.size,
              type: employmentCertificate.type,
            };
          }
        } catch (uploadError) {
          console.error(
            "Employment certificate upload exception:",
            uploadError
          );
        }
      }

      // 문서 정보 업데이트 (파일이 업로드된 경우에만)
      if (Object.keys(documentsInfo).length > 0) {
        const { error: updateError } = await supabase
          .from("users")
          .update({ documents: documentsInfo })
          .eq("id", newUser.id);
