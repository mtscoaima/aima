  ...
  try {
    ...
    if (campaign.status !== "ACTIVE") {
      return NextResponse.json(
        {
          success: false,
          message: "진행 중인 캠페인만 일시정지할 수 있습니다.",
        },
        { status: 400 }
      );
    }
    ...
    if (updateError) {
      console.error("캠페인 일시정지 오류:", updateError);
      return NextResponse.json(
        { success: false, message: "캠페인 일시정지 처리에 실패했습니다." },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: "캠페인이 성공적으로 일시정지되었습니다.",
    });
  } catch (error) {
    console.error("캠페인 일시정지 오류:", error);
    return NextResponse.json(
