...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isFirst) {
      ...
      console.log('[WIDE_ITEM_LIST] 첫 번째 아이템 이미지 업로드 시도...');
      const response = await fetch(`${MTS_TEMPLATE_API_URL}/mts/api/direct/image/wideItemList/first`, {
        method: 'POST',
        body: mtsFormData,
      });
      ...
      if (contentType && contentType.includes('text/html')) {
        ...
        console.error('[WIDE_ITEM_LIST] HTML 응답 받음 (처음 500자):', htmlText.substring(0, 500));
        return NextResponse.json(
          {
            success: false,
            error: 'WIDE_ITEM_LIST 이미지 업로드 API가 올바르지 않습니다. MTS에서 브랜드 메시지 권한을 확인해주세요.',
          },
          { status: 400 }
        );
      }
      ...
      console.log('[WIDE_ITEM_LIST] 첫 번째 아이템 응답:', result);
      ...
      console.log('[WIDE_ITEM_LIST] 서브 아이템 이미지 업로드 시도...');
      const response = await fetch(`${MTS_TEMPLATE_API_URL}/mts/api/direct/image/wideItemList`, {
        method: 'POST',
        body: mtsFormData,
      });
      ...
      const result = await response.json();
      console.log('[WIDE_ITEM_LIST] 서브 아이템 응답:', JSON.stringify(result, null, 2));

      // 성공 코드: '0000' (전체 성공), '6000' (부분 성공)
      if (['0000', '6000'].includes(result.code) && result.result?.success?.length > 0) {
        // MTS API 문서 명세대로 result.success 배열에서 url 필드 추출
        const firstSuccess = result.result.success[0];
        const imageUrl = firstSuccess.url;

        if (imageUrl) {
          console.log('[WIDE_ITEM_LIST] 서브 아이템 이미지 URL:', imageUrl);
          console.log('[WIDE_ITEM_LIST] 업로드 필드:', firstSuccess.formField);

          // 부분 성공인 경우 실패 항목 로깅
          if (result.code === '6000' && result.result.failure?.length > 0) {
            console.warn('[WIDE_ITEM_LIST] 일부 이미지 업로드 실패:', result.result.failure);
          }
    ...
    console.error('WIDE_ITEM_LIST 이미지 업로드 오류:', error);
    return NextResponse.json(
      {
