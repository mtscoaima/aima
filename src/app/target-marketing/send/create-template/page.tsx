...
// GPT API 키
const GPT_API_KEY = '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';
...
export default function TemplateCreatePage() {
  ...
  const handleModeChange = (newMode: 'basic' | 'ai') => {
    ...
    if (newMode === 'ai') {
      ...
      if ((titleText || subTitleText || bodyText) && !promptDescription.trim()) {
        const currentData = [
        ...
        if (currentData) {
          setPromptDescription(`현재 작성 중인 템플릿 내용을 바탕으로 개선해주세요:\n${currentData}`);
        }
      }
    }
  };
  ...
  const startChat = async () => {
    ...
    if (messages.length === 0) {
      ...
      const initialMessage: Message = {
        role: 'assistant',
        content: '안녕하세요! 마케팅 템플릿 생성을 도와드리겠습니다. 어떤 템플릿을 만들고 싶으신가요?'
      };
  ...
  const generateImage = async () => {
    if (!imagePrompt) {
      alert('이미지 생성을 위한 프롬프트가 필요합니다. AI 템플릿을 먼저 생성해주세요.');
      return;
    }
    ...
    try {
      ...
      if (data.data && data.data.length > 0) {
        ...
        setMessages(prev => [...prev, { 
          role: 'assistant', 
          content: '이미지가 성공적으로 생성되었습니다! 템플릿 미리보기에서 확인해보세요.' 
        }]);
      }
    } catch (error) {
      ...
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: '죄송합니다. 이미지 생성 중 오류가 발생했습니다. 다시 시도해주세요.' 
      }]);
    } finally {
                ...
                <div className="editor-section">
                  ...
                  <div className="ai-helper-container">
                      ...
                      <div className="tone-tags">
                        {[
                          ...
                          <button
                            key={tone.value}
                            type="button"
                            className={`tone-tag ${selectedTones.includes(tone.value) ? 'selected' : ''}`}
                    ...
                    <div className="ai-tips">
                      <h3>💡 AI 도우미 사용 팁</h3>
                      <ul>
                        <li>구체적인 설명을 제공할수록 더 좋은 결과를 얻을 수 있습니다.</li>
                        <li>특별한 프로모션이나 할인 정보를 포함하면 전환율이 높아집니다.</li>
                ...
                <div className="chat-container">
                  ...
                  <div className="chat-actions">
                    {imagePrompt && (
                      ...
                      >
                        {isGeneratingImage ? '이미지 생성중...' : '🖼️ AI 이미지 생성하기'}
                      </button>
                    )}
                  </div>
