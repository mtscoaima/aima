...
export default function CampaignStatusPage() {
  ...
  function generateDailyStats(
    ...
    for (let i = 0; i < days; i++) {
      ...
      const sent =
        Math.floor(Math.random() * (maxSent - minSent + 1)) + minSent;
      const conversionRate =
        Math.random() * (maxConversion - minConversion) + minConversion;
      ...
      campaign.dailyStats.forEach((stat) => {
        if (statsMap.has(stat.date)) {
          ...
          statsMap.set(stat.date, {
            ...
            conversionRate:
              newSent > 0
                ? parseFloat(((newResponded / newSent) * 100).toFixed(1))
                : 0,
          });
        }
      });
          ...
          <>
            <div className="summary-stats">
              ...
              <div className="stat-card total-sent">
                <div className="stat-value">
                  {campaigns
                    .reduce((sum, campaign) => sum + campaign.metrics.sent, 0)
                    .toLocaleString()}
                </div>
                  ...
                  {campaignTotalStats.map((campaign) => (
                    <div key={campaign.id} className="table-row">
                      ...
                      <div className="col-period">
                        {campaigns.find((c) => c.id === campaign.id)?.startDate}{" "}
                        ~ {campaigns.find((c) => c.id === campaign.id)?.endDate}
                      </div>
                      <div className="col-sent">
                        {campaign.sent.toLocaleString()}
                      </div>
                      <div className="col-responded">
                        {campaign.responded.toLocaleString()}
                      </div>
                      <div className="col-conversion">
