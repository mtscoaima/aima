  ...
  useEffect(() => {
    ...
    const demoTemplates: Template[] = [
      ...
      },
      {
        ...
        imageUrl: 'https://images.unsplash.com/photo-1556742212-5b321f3c261b?w=600&auto=format&fit=crop&q=60',
        status: 'active',
        type: 'target'
      },
      {
        ...
        imageUrl: 'https://images.unsplash.com/photo-1513001900722-370f803f498d?w=600&auto=format&fit=crop&q=60',
        status: 'inactive',
        type: 'target'
      },
      {
      ...
      {isLoading ? (
        ...
        <div className="templates-grid">
          {filteredTemplates.map((template) => (
            <div 
              key={template.id} 
              className={`template-card ${template.status === 'inactive' ? 'inactive-template' : 'active-template'}`}
            >
              ...
              <div className="template-info">
                ...
                <div className="template-meta">
                  <span className="template-date">작성일: {formatDate(template.createdAt)}</span>
                  <span className="template-type">{template.type === 'target' ? '타겟마케팅' : '문자메시지'}</span>
                </div>
              </div>
              <div className="template-actions">
                ...
                <div className="button-container">
                  <Link href={`/target-marketing/send/create-template?edit=${template.id}`}>
              ...
              {template.status === 'inactive' && <div className="inactive-overlay"></div>}
            </div>
          ))}
        </div>
      ) : (
