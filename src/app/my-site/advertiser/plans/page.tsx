        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div className="mt-1 flex items-center">
              ...
              {currentPlan === 'prepaid' && (
                <span className="ml-4 text-gray-700">잔액: <span className="font-medium text-blue-600">{formatCurrency(prepaidData.balance)}</span></span>
              )}
            </div>
          </div>
          <button 
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6 border-t-4 border-t-green-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium text-xl text-blue-600">{formatCurrency(prepaidData.balance)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">포인트 잔액</p>
                <p className="font-medium text-xl text-green-600">{formatCurrency(prepaidData.pointBalance)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{formatCurrency(prepaidData.lastChargeAmount)}</p>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">충전일시</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">금액</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">결제수단</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">상태</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">영수증</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {prepaidData.chargeHistory.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{getPaymentMethodText(item.paymentMethod)}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <a href={item.receiptUrl} className="text-blue-600 hover:text-blue-800">보기</a>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">증정일시</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">금액</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">사유</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">만료일</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">사용일시</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">금액</th>
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6 border-t-4 border-t-green-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{postpaidData.companyInfo.companyName}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">사업자등록번호</p>
                <p className="font-medium">{postpaidData.companyInfo.businessNumber}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">대표자명</p>
                <p className="font-medium">{postpaidData.companyInfo.representativeName}</p>
              </div>
              <div className="md:col-span-2">
                <p className="text-sm text-gray-600">주소</p>
                <p className="font-medium">{postpaidData.companyInfo.address}</p>
              </div>
            </div>
            <div className="mt-4">
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{getPaymentMethodText(postpaidData.paymentInfo.paymentMethod)}</p>
              </div>

              {postpaidData.paymentInfo.paymentMethod === 'card' && postpaidData.paymentInfo.cardInfo && (
                <>
                  <div>
                    <p className="text-sm text-gray-600">카드사</p>
                    <p className="font-medium">{postpaidData.paymentInfo.cardInfo.cardCompany}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">카드번호</p>
                    <p className="font-medium">**** **** **** {postpaidData.paymentInfo.cardInfo.lastFourDigits}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">유효기간</p>
                    <p className="font-medium">{postpaidData.paymentInfo.cardInfo.expiryDate}</p>
                  </div>
                </>
              )}

              {postpaidData.paymentInfo.paymentMethod === 'bank' && postpaidData.paymentInfo.bankInfo && (
                <>
                  <div>
                    <p className="text-sm text-gray-600">은행명</p>
                    <p className="font-medium">{postpaidData.paymentInfo.bankInfo.bankName}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">계좌번호</p>
                    <p className="font-medium">{postpaidData.paymentInfo.bankInfo.accountNumber}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">예금주</p>
                    <p className="font-medium">{postpaidData.paymentInfo.bankInfo.accountHolder}</p>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{postpaidData.invoiceInfo.email}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">담당자명</p>
                <p className="font-medium">{postpaidData.invoiceInfo.managerName}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">연락처</p>
                <p className="font-medium">{postpaidData.invoiceInfo.contactNumber}</p>
              </div>
              <div className="md:col-span-2">
                <p className="text-sm text-gray-600">주소</p>
                <p className="font-medium">{postpaidData.invoiceInfo.address}</p>
              </div>
            </div>
            <div className="mt-4">
              <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">청구일</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">금액</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">상태</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">납부일</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">세금계산서</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {postpaidData.billingHistory.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <a href={item.invoiceUrl} className="text-blue-600 hover:text-blue-800">보기</a>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <div className="p-4 bg-gray-50 rounded-md">
          <p className="text-sm text-gray-600 mb-2">
            선불 요금제와 후불 요금제 간 변경을 원하시면 고객센터로 문의해주세요.
          </p>
          <p className="text-sm text-gray-600 mb-2">
          ...
          <div className="mt-4">
            <Link href="/customer-service/inquiry" className="text-blue-600 hover:text-blue-800 text-sm font-medium">
              고객센터 문의하기
            </Link>
          </div>
        </div>
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-4">
                      <p className="text-sm text-gray-500 mb-4">
                        변경하실 요금제를 선택해주세요. 요금제 변경에는 관리자 승인이 필요할 수 있습니다.
                      </p>
                      
                      <div className="mt-4 space-y-3">
                        <div className={`p-4 border rounded-lg ${currentPlan === 'prepaid' ? 'bg-blue-50 border-blue-300' : 'hover:bg-gray-50 cursor-pointer'}`}
                             onClick={() => handleChangePlan('prepaid')}>
                          <div className="flex items-center">
                            <div className={`h-5 w-5 rounded-full ${currentPlan === 'prepaid' ? 'bg-blue-600' : 'bg-gray-200'} flex items-center justify-center mr-3`}>
                              {currentPlan === 'prepaid' && <span className="text-white text-xs">✓</span>}
                            </div>
                            <h4 className="text-md font-medium">선불 요금제</h4>
                          </div>
                          <p className="mt-1 ml-8 text-sm text-gray-600">미리 충전 후 사용하는 방식입니다. 충전 금액의 5%가 포인트로 적립됩니다.</p>
                        </div>
                        
                        <div className={`p-4 border rounded-lg ${currentPlan === 'postpaid' ? 'bg-blue-50 border-blue-300' : 'hover:bg-gray-50 cursor-pointer'}`}
                             onClick={() => handleChangePlan('postpaid')}>
                          <div className="flex items-center">
                            <div className={`h-5 w-5 rounded-full ${currentPlan === 'postpaid' ? 'bg-blue-600' : 'bg-gray-200'} flex items-center justify-center mr-3`}>
                              {currentPlan === 'postpaid' && <span className="text-white text-xs">✓</span>}
                            </div>
                            <h4 className="text-md font-medium">후불 요금제</h4>
                          </div>
                          <p className="mt-1 ml-8 text-sm text-gray-600">이용한 만큼 월별로 청구됩니다. 기업 신용 평가가 필요합니다.</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button 
                  ...
                  onClick={() => alert('요금제 변경 요청이 접수되었습니다. 관리자 검토 후 진행됩니다.')}
                >
                  변경 요청
                </button>
                <button 
