...
export default function BusinessVerificationPage() {
  ...
  const [verificationData, setVerificationData] =
    useState<BusinessVerificationData>({
      status: "processing",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-medium">
                {verificationData.representativeName}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="font-medium">
                {verificationData.customerServiceNumber}
              </p>
            </div>
            <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between p-3 border rounded-lg">
              <div>
                <div className="flex items-center">
                  ...
                  <span
                    className={`ml-2 px-2 py-0.5 rounded-full text-xs ${getDocumentStatusBadgeStyle(
                      verificationData.documents.businessRegistration.status
                    )}`}
                  >
                ...
                <p className="text-sm text-gray-600 mt-1">
                  파일명:{" "}
                  {verificationData.documents.businessRegistration.fileName}
                </p>
                <p className="text-sm text-gray-600">
                  제출일:{" "}
                  {verificationData.documents.businessRegistration.submittedAt}
                </p>
              </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between p-3 border rounded-lg">
                <div>
                  <div className="flex items-center">
                    ...
                    <span
                      className={`ml-2 px-2 py-0.5 rounded-full text-xs ${getDocumentStatusBadgeStyle(
                        verificationData.documents.employmentCertificate.status
                      )}`}
                    >
                      {getDocumentStatusText(
                        verificationData.documents.employmentCertificate.status
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    파일명:{" "}
                    {verificationData.documents.employmentCertificate.fileName}
                  </p>
                  <p className="text-sm text-gray-600">
                    제출일:{" "}
                    {
        ...
        <div className="bg-white rounded-lg shadow p-4 mb-6 border-t-4 border-t-purple-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-medium">
                {verificationData.taxInvoiceInfo.email}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">담당자명</p>
              <p className="font-medium">
                {verificationData.taxInvoiceInfo.managerName}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-600">연락처</p>
              <p className="font-medium">
                {verificationData.taxInvoiceInfo.contactNumber}
              </p>
            </div>
          </div>
          <div className="mt-4">
        ...
        <div className="bg-white rounded-lg shadow p-4 border-t-4 border-t-gray-500">
          ...
          <div className="mt-4 p-3 bg-yellow-50 rounded-md">
            <p className="text-sm text-yellow-800">
              <strong>참고:</strong> 인증 처리 문의는 고객센터 (1544-9876)로
              연락주시기 바랍니다.
            </p>
          </div>
        </div>
